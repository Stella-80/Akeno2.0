...
class Config(object):
    ...
    # RECOMMENDED
    SQLALCHEMY_DATABASE_URI = 'postgres://postgres:stella123@database-2.cuzno6euao1o.us-east-2.rds.amazonaws.com:5432/catuserbot'  # needed for any database modules
    ...
    SUDO_USERS = [1603318426]  # List of id's -  (not usernames) for users which have sudo access to the bot.
    DEV_USERS = [1603318426]  # List of id's - (not usernames) for developers who will have the same perms as the owner
    SUPPORT_USERS = [1603318426]  # List of id's (not usernames) for users which are allowed to gban, but can also be banned.
    WHITELIST_USERS = [1603318426]  # List of id's (not usernames) for users which WONT be banned/kicked by the bot.
